...
contract LpBond is IERC3475, IERC3475EXTENSION {
     ...
     */
    constructor() { 
        _classes[1]._valuesId[1] = "LPSM";    
        _classes[1]._values["LPSM"].stringValue = "Collator Bond Token";    
        }
    ...
    function transferFrom(
        ...
        require(
            _from != address(0),
            "LPBond: can't transfer from the zero address"
        );
        require(
            _to != address(0),
            "LPBond:use burn() instead"
        );
        require(
            msg.sender == _from ||
            isApprovedFor(_from, msg.sender),
            "LPBond:caller-not-owner-or-approved"
        );
    ...
    function transferAllowanceFrom(
        ...
        require(
            _from != address(0),
            "LPBond: can't transfer allowed amt from zero address"
        );
        require(
            _to != address(0),
            "LPBond: use burn() instead"
    ...
    override
    {
        ...
        for (uint256 i = 0; i < len; i++) {
            require(
                _to != address(0),
                "LPBond: can't issue to the zero address"
            );
    ...
    override
    {
        require(
            _from != address(0),
            "LPBond: can't redeem from the zero address"
        );
        uint256 len = _transactions.length;
        for (uint256 i = 0; i < len; i++) {
    ...
    override
    {
        require(
            _from != address(0),
            "LPBond: can't burn from the zero address"
        );
        require(
            msg.sender == _from ||
            isApprovedFor(_from, msg.sender),
    ...
    function balanceOf(
        ...
        require(
            account != address(0),
            "LPBond: balance query for the zero address"
        );
    ...
    function _transferFrom(
        ...
        require(
            ...
            "LPBond: not enough bond to transfer"
        );
